...
def setup_test_args():
    ...
    sys.argv = ["parabeagle", "--client-type", "ephemeral"]
    yield
...
def test_persistent_client_creation(mock_persistent_client, mock_env_vars):
    ...
    # Set up command line args
    sys.argv = ["parabeagle", "--client-type", "persistent", "--data-dir", "/test/data/dir"]
...
def test_required_args_for_http_client():
    ...
    with patch("argparse.ArgumentParser.error") as mock_error:
        ...
        sys.argv = ["parabeagle", "--client-type", "http"]
...
def test_required_args_for_cloud_client():
    ...
    with patch("argparse.ArgumentParser.error") as mock_error:
        ...
        sys.argv = ["parabeagle", "--client-type", "cloud"]
...
async def test_delete_documents_success():
    ...
    try:
        ...
        # 2. Delete documents
        delete_result = await mcp.call_tool(
            "chroma_delete_documents", {"collection_name": collection_name, "ids": doc_ids}
        )
        ...
        get_result_raw = await mcp.call_tool(
            "chroma_get_documents",
            {"collection_name": collection_name, "ids": doc_ids, "include": ["documents"]},
        )
        assert len(get_result_raw) == 1
        get_result = json.loads(get_result_raw[0].text)
        assert isinstance(get_result, dict)
...
async def test_delete_documents_invalid_args():
    ...
    try:
        ...
        with pytest.raises(ToolError, match="The 'ids' list cannot be empty."):
            await mcp.call_tool(
                "chroma_delete_documents", {"collection_name": collection_name, "ids": []}
            )
...
async def test_delete_documents_collection_not_found():
    ...
    with pytest.raises(ToolError, match="Failed to get collection"):
        await mcp.call_tool(
            ...
            {"collection_name": "non_existent_collection", "ids": ["doc1"]},
        )
...
async def test_delete_documents_id_not_found():
    ...
    try:
        ...
        delete_result = await mcp.call_tool(
            "chroma_delete_documents",
            {"collection_name": collection_name, "ids": ["non_existent_id"]},
        )
        # Check the success message (even though the ID didn't exist)
        assert len(delete_result) == 1
        assert "Successfully deleted" in delete_result[0].text
...
async def test_peek_collection_success():
    ...
    try:
        ...
        limited_result = await mcp.call_tool(
            "chroma_peek_collection", {"collection_name": collection_name, "limit": 2}
        )
        assert len(limited_result) == 1
...
async def test_get_collection_count_success():
    ...
    try:
        ...
        result = await mcp.call_tool(
            "chroma_get_collection_count", {"collection_name": collection_name}
        )
        assert isinstance(result, list)
        assert len(result) == 1
...
async def test_modify_collection_success():
    ...
    try:
        ...
        # Test modifying name
        name_result = await mcp.call_tool(
            "chroma_modify_collection", {"collection_name": collection_name, "new_name": new_name}
        )
        ...
        metadata_result = await mcp.call_tool(
            "chroma_modify_collection",
            {"collection_name": new_name, "new_metadata": {"test_key": "test_value"}},
        )
...
async def test_add_documents_invalid_args():
    ...
    try:
        ...
        with pytest.raises(ToolError, match="The 'documents' list cannot be empty"):
            await mcp.call_tool(
                "chroma_add_documents",
                {"collection_name": collection_name, "documents": [], "ids": ["doc1"]},
            )
...
async def test_query_documents_invalid_args():
    ...
    try:
        ...
        with pytest.raises(ToolError, match="The 'query_texts' list cannot be empty"):
            await mcp.call_tool(
                "chroma_query_documents",
                {"collection_name": collection_name, "query_texts": [], "n_results": 5},
            )
...
async def test_get_documents_success():
    ...
    try:
        ...
        filter_result = await mcp.call_tool(
            "chroma_get_documents",
            {
                "collection_name": collection_name,
                "where": {"category": "A"},
        ...
        paginated_result = await mcp.call_tool(
            "chroma_get_documents",
            {"collection_name": collection_name, "limit": 2, "offset": 1, "include": ["documents"]},
        )
...
async def test_get_documents_collection_not_found():
    ...
    with pytest.raises(ToolError, match="Failed to get documents from collection"):
        await mcp.call_tool(
            "chroma_get_documents", {"collection_name": "non_existent_collection", "ids": ["doc1"]}
        )
