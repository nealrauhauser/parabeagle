...
def remove_pdf_from_collection(data_dir, collection_name, pdf_path, dry_run=False):
    ...
    try:
        ...
        print(f"Looking for documents from: {pdf_path}")
        ...
        if not matching_ids:
            print(f"No documents found from {pdf_name} in collection '{collection_name}'")
            return 0
        
        print(f"Found {len(matching_ids)} documents from {pdf_name}")
        
        if dry_run:
            print("DRY RUN - would delete:")
            for doc_id in matching_ids:
                print(f"  - {doc_id}")
            print(f"Total: {len(matching_ids)} documents")
            return 0
        
        # Delete the matching documents
        collection.delete(ids=matching_ids)
        
        print(f"Successfully deleted {len(matching_ids)} documents from collection '{collection_name}'")
        print(f"Collection now has {collection.count()} total documents")
        return 0
        
    except Exception as e:
        print(f"Error removing PDF from collection: {e}")
        return 1
